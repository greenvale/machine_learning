...
neural_network::neural_network(const std::vector<int>& shape,
	const std::vector<std::string>& activate_types,
	const std::string& loss_type) :
	m_shape(shape), m_activate_types(activate_types), m_loss_type(loss_type)
{
	m_N = m_shape.size() - 1;
	...
	for (int i = 0; i < m_N; ++i)
	{
		m_z.push_back(std::vector<double>(m_shape[i+1], 0.0));
		m_a.push_back(std::vector<double>(m_shape[i+1], 0.0));
		m_W.push_back(std::vector<double>(m_shape[i]*m_shape[i+1], 0.5));
		m_b.push_back(std::vector<double>(m_shape[i+1], 0.5));
		
		m_dJ_da.push_back(std::vector<double>(m_shape[i+1]));
		m_da_dz.push_back(std::vector<double>(m_shape[i+1]));
		m_dJ_dW.push_back(std::vector<double>(m_shape[i]*m_shape[i+1]));
		m_dJ_db.push_back(std::vector<double>(m_shape[i+1]));
	}
}
...
void neural_network::fwd_pass(const std::vector<double>& x)
{
	for (int i = 0; i < m_N; ++i)
	{
		...
		for (int j = 0; j < m_shape[i+1]; ++j)
		{
			m_z[i][j] = m_b[i][j];
			for (int k = 0; k < m_shape[i]; ++k)
			{
				if (i > 0)
					m_z[i][j] += m_W[i][k*m_shape[i+1] + j] * m_a[i-1][k];
				else
					m_z[i][j] += m_W[i][k*m_shape[i+1] + j] * x[k];
			}
		}
...
void neural_network::backprop(const std::vector<double>& y)
{
	...
	for (int i = m_N - 2; i >= 0; i--)
	{
		...
		{
			m_dJ_da[i][j] = 0.0;
			for (int k = 0; k < m_shape[i+2]; ++k)
			{
				m_dJ_da[i][j] += m_dJ_da[i+1][k] * m_da_dz[i+1][k] * m_W[i+1][j*m_shape[i+2] + k];
			}
		}
	}
	...
	for (int i = 0; i < m_N; ++i)
	{
		...
		{
			for (int k = 0; k < m_shape[i+1]; ++k)
			{
				m_dJ_dW[i][j*m_shape[i+1] + k] = m_dJ_da[i][k] * m_da_dz[i][k] * m_a[i][j];
			}
		}
	}
	...
	for (int i = 0; i < m_N; ++i)
	{
		for (int j = 0; j < m_shape[i]; ++j)
		{
			for (int k = 0; k < m_shape[i+1]; ++k)
			{
				m_W[i][j*m_shape[i+1] + k] += -0.1 * m_dJ_dW[i][j*m_shape[i+1] + k];
			}
		}
		for (int j = 0; j < m_shape[i+1]; ++j)
		{
...
void neural_network::print()
{
	for (int i = 0; i < m_N; ++i)
	{
		...
		for (int j = 0; j < m_shape[i]; ++j)
		{
			...
			{
				std::cout.precision(2);
				std::cout << m_W[i][j*m_shape[i+1] + k] << "\t";
			}
			for (int k = 0; k < m_shape[i+1]; ++k)
			{
				std::cout.precision(2);
				std::cout << m_dJ_dW[i][j*m_shape[i+1] + k] << "\t";
			}
			std::cout << "\n";
		}
		std::cout << "\nb";
