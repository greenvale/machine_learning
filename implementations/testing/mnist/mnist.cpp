...
std::tuple<T*, std::vector<uint32_t>, uint32_t, uint32_t> read_IDX(const std::string& path)
{
    ...
    //std::cout << std::bitset<32>(magic_num) << std::endl;
    //std::cout << std::bitset<32>(swap(magic_num)) << std::endl;
...
public:
    ...
    void print_char(const std::string& set, const int& n);

    void eval(const std::string& set, const int& n);
    ...
    const std::string& loss_type)
{
    ...
    for (int i = 0; i < 10000; ++i)
    {
        for (int j = 0; j < 28*28; ++j)
        {
            m_test_x[i][j] = ((double)train_imgs_arr[28*28*i + j]) / 255.0;
        }
        m_test_y[i][test_labels_arr[i]] = 1.0;
    }

    // initialise neural network
    m_nn = new neural_network(shape, activate_types, loss_type);
}
...
void MNIST::print_char(const std::string& set, const int& n)
{
    std::cout << "\n";
    for (int i = 0; i < 28; ++i)
    {
...
void MNIST::eval(const std::string& set, const int& n)
{
    m_nn->fwd_pass(m_train_x[n]);
    std::cout << m_nn->get_output() << std::endl;
}
...
{   
    MNIST mnist("./data/train-images.idx3-ubyte", 
        ...
        {28*28, 10, 10}, {"relu", "softmax"}, "crossentropy");

    mnist.print_char("train", 1);
